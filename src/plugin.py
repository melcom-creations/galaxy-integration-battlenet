...
# Ensure modules directory is in sys.path first, before any imports
...
class BNetPlugin(Plugin):
    def __init__(self, reader, writer, token):
        ...
        try:
            self.oauth_credentials = read_oauth_config()
        except OAuthConfigError as error:
            log.warning("Battle.net OAuth configuration is unavailable: %s", error)
    ...
    async def _setup_next_step(self):
        try:
            ...
            log.warning("Battle.net OAuth setup file could not be prepared: %s", error)
        ...
        return NextStep(
            "web_session",
            {
                ...
                "window_width": 760,
                "window_height": 900,
                "start_uri": self.setup_server.base_url,
                "end_uri_regex": self.setup_server.end_uri_regex,
            },
        )
    ...
    async def pass_login_credentials(self, step, credentials, cookies):
        ...
        if setup_credentials is not None:
            try:
                ...
                log.warning("Battle.net OAuth configuration could not be saved: %s", error)
                raise InvalidCredentials("Battle.net OAuth credentials could not be saved") from error
            finally:
                await self.setup_server.stop()
