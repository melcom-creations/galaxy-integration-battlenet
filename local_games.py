...
class LocalGames():
    ...
    def _add_classic_game(self, game, key):
        if game.registry_path:
            try:
                # Query the 32-bit registry view to find classic games on 64-bit Windows.
                with winreg.OpenKey(key, game.registry_path, 0, winreg.KEY_READ | winreg.KEY_WOW64_32KEY) as game_key:
    ...
    def _find_classic_games(self):
        ...
        if SYSTEM == Platform.WINDOWS:
            try:
                ...
                # Query the 32-bit registry branch (Wow6432Node) on 64-bit Windows.
                with winreg.OpenKey(reg, WINDOWS_UNINSTALL_LOCATION, 0, winreg.KEY_READ | winreg.KEY_WOW64_32KEY) as key:
        ...
        with self.installed_classic_games_lock:
    ...
    async def parse_local_classic_games(self):
        ...
        # Use thread.is_alive() to wait for completion.
        if not self._classic_games_thread or not self._classic_games_thread.is_alive():
